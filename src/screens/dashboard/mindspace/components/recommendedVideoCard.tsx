...
export const RecommendedVideoCard = ({ item } : { item: any }) => {
  const navigation = useNavigation<NavigationProp<DashboardParamList, 'MindSpace'>>();
  return (
    <TouchableOpacity style={styles.recommendedSectionItemContainer} onPress={() => navigation.navigate('CbtVideo', { header: item.title, backgroundImageUri: item.background_url, video_uri: item.content_url, id: item?.id, is_complete: item?.play_history?.is_complete, duration: item?.play_history?.current_duration }) }>
    <Image
        source={{ uri: item?.background_url }}
        resizeMode="cover"
        style={styles.recommendedVideoStyle}
      />
    <LinearGradient
        colors={['rgba(38, 28, 64, 0.2)', '#261C40']}
        style={styles.recommendedVideoStyleGradient}
    >
