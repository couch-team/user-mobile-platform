...
const CbtAudio = ({ navigation: { goBack } }: Props) => {
    ...
    const fetchAudio = async() => {
        try{
            ...
            const { sound: newSound } = await Audio.Sound.createAsync({ uri: params?.audio_uri });
            setSound(newSound);
            ...
            >
                ...
                style={[styles.gradient, { paddingTop: top } ]}>
                    ...
                    <View style={styles.contentContainer}>
                        ...
                        <View style={styles.audioControls}>
                            <TouchableOpacity style={styles.audioLength} onPress={(e) => hasStarted && handleMoveToPosition(e)}>
                                <View style={[styles.audioPlayedLength, isLoadingAudio ? { width: 0 } : { width: `${(playedDuration/duration) * 100}%` }]}></View>
                            </TouchableOpacity>
                            <View style={styles.audioControlsLabel}>
                                <Text style={styles.audioControlsText}>{dayjs(playedDuration).format('mm:ss')}</Text>
                                <Text style={styles.audioControlsText}>{ isLoadingAudio ? <ActivityIndicator size="small" color={Colors.WHITE}/> : dayjs(duration).format('mm:ss')}</Text>
                            </View>
                        </View>
                        {
                            hasStarted
                                ...
                                <TouchableOpacity style={styles.button} onPress={() => { playAudio() }} disabled={isLoadingAudio}>
                                     { isLoadingAudio ? <ActivityIndicator size="small" color={Colors.WHITE}/> : <Text style={styles.buttonText}>Get Started</Text> }
                                </TouchableOpacity>
                        }
                    </View>
              </LinearGradient>
              {
                ...
                <LinearGradient
                    ...
                    >
                            <TouchableOpacity style={styles.replayOption} onPress={() => handleRestart() }>
                                <REPLAY_ICON/>
                            </TouchableOpacity>
