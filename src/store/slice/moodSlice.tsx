...
const MoodSlice = createSlice({
    ...
    initialState,
    reducers: {
        setMoods: (state, { payload }:PayloadAction<MoodType[]>) => { state.moods = payload },
...
export const { setMoods, setMoodsCurrentPage, setMoodsLastPage, setHasFetchedMoods, setIsFetchingMoods } = MoodSlice.actions;
...
const debounceFetchMoods = debounce(async(current_page, dispatch) => {
    try{
        ...
        if($api.isSuccessful(response)){
            dispatch(setMoods(response?.data?.results))
